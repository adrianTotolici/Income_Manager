    ...
    public RpcResponse doInvoke(RpcRequest request, Object target, Object context) {
        ...
        try {
            ...
            for (int i = 0, j = 0; i < processedParams.length; i++) {
                if (paramProcessor.isInjectedParam(paramAnnotations[i]))
                    processedParams[i] = paramProcessor.injectParam(paramTypes[i], paramAnnotations[i], context);
                else
                    processedParams[i] = paramProcessor.processParam(params[j++], paramTypes[i], paramAnnotations[i], context);
            }
