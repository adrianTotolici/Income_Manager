...
public class RmiService extends MessageProcessor {
    ...
    protected void process(Message m) {
        if (m.contentOfType(Call.class)) handle(m.getContentAs(Call.class, gson));
            ...
            handle(m.getContentAs(RpcResponse.class, gson), m.dests.get(0), Arrays.asList(m.src));
        else if (m.contentOfType(DeliveryFailure.class))
            handle(m.getContentAs(DeliveryFailure.class, gson), m.dests.get(0), m.src);
                ...
                if (response.id != null) t.send(new Message(dest, Arrays.asList(src), response));
                    ...
                    if (response.error != null) {
                        ...
                        if (response.error.equals(RpcError.INVOCATION_EXCEPTION))
                            response.error.data.getValue(Exception.class, gson).printStackTrace();
                    }
                }
            }
        }
    ...
    public void periodicCleanup() {
        ...
        if (pendingCalls.size() < count)
            System.err.println("INFO: cleanup pending calls " + count + " -> " + pendingCalls.size());
