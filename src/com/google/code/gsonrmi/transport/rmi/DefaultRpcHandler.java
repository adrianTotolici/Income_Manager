...
public class DefaultRpcHandler implements RpcHandler {
    ...
    public void handle(RpcResponse response, Route dest, List<Route> srcs, Callback callback) {
        ...
        for (Route src : srcs) {
            ...
            if (r.error != null) {
                ...
                if (r.error.equals(RpcError.INVOCATION_EXCEPTION))
    ...
    public void periodicCleanup() {
        ...
        if (sessions.size() < count)
            System.err.println("INFO: " + target.getClass().getSimpleName() + "-" + (target.hashCode() % 1000) + " cleanup sessions " + count + " -> " + sessions.size());
    }
